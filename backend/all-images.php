...
if(isset($_GET['id']) && !empty($_GET['id'])){
    $query = "SELECT thumbnail,id,tags,information,mid,live FROM search WHERE " . $_GET['id'] . " ORDER BY id DESC";
    $params = 'id=' . $_GET['id'];
} else if(isset($_GET['tag']) && !empty($_GET['tag'])) {
    $query = "SELECT thumbnail,id,tags,information,mid,live FROM search WHERE tags like '%" . mysql_escape_string($_GET['tag']) . "%' ORDER BY id DESC";
    $params = 'tag=' . $_GET['tag'];
} else if(isset($_GET['live']) && !empty($_GET['live']) && $_GET['live'] === 'false'){
    $query = "SELECT thumbnail,id,tags,information,mid,live FROM search WHERE live = '0' ORDER BY id DESC";
    $params = 'live=false';
} else {
    $query = "SELECT thumbnail,id,tags,information,mid,live FROM search WHERE 1=1 ORDER BY id DESC";
    $params = 'all=true';
}
...
<?php ob_start(); ?>
    ...
    if($num == 1){
        echo "Currently on page $page of " . $pager->numPages . ", images " . ($pager->firstRecord + 1) . " through " . ($pager->lastRecord) . "</h3>";
    }else if($num != 0) {
    ...
    for ($i = $pager->firstRecord; $i <= $pager->lastRecord; $i++) {
        ...
        echo "<tr><td><br><br><div style=\"width: 224px; height: 224px;\"><a id='thumb$id' href=\"" . WEB_ROOT . "$mid\" rel=\"lightbox\"><img src=\"" . WEB_ROOT . "$thumbnail\" /></a></div>
            ...
            <tr><td></td><td align=\"right\"><p>Information:</p> <textarea class = \"textarea\" name=\"$picinfo\" id=\"$picinfo\" rows=\"5\" cols=\"40\" readonly=\"readonly\">$info</textarea><br>
            <input type=\"checkbox\" name=\"live_$id\" value=\"live\" $live/>Ready for viewing?<br><br>
